...
| **DB-03** | **P1** | **Seeder and DB are the only sources of image truth** | There is no canonical registry of which image belongs to which service/product. Mappings live in individual seeders (`HomeHealthcareSeeder`, `IVTherapySeeder`, etc.). | Create a single `ImageRegistry` class or config array that maps every service/product slug to its canonical image. Seeders and migrations both read from it. |
...
| **DB-05** | **P1** | **Seeders use brittle title matching** | `resolveImage()` in `HomeHealthcareSeeder` matches exact human-readable titles. Renaming a service silently breaks its image mapping. | Map images by stable slug or SKU, not by display title. Treat titles as translatable/changeable labels. |
