...
class VendorServiceAssignmentController extends Controller
{
    ...
    {
        return response()->json(['success' => true, 'assignment' => $this->service->setVendorServiceAssignment($vendorId, $serviceId, $request->input('enabled'))]);
    }

    public function bulk(Request $request, string $vendorId): JsonResponse
    {
        return response()->json($this->service->bulkSetVendorServiceAssignments($vendorId, $request->input('serviceIds', []), $request->input('enabled', true)));
    }
}
